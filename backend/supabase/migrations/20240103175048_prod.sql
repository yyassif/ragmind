...
BEGIN
    ...
    IF NOT NEW.models ? 'gpt-3.5-turbo-1106' THEN
        NEW.models := NEW.models || '["gpt-4o", "gpt-3.5-turbo"]'::jsonb;
    END IF;
...
CREATE TRIGGER update_max_brains_trigger AFTER INSERT ON public.user_settings FOR EACH ROW EXECUTE FUNCTION update_max_brains();
