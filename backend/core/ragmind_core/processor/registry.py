...
known_processors: dict[FileExtension | str, ProcEntry] = {
    FileExtension.txt: ProcEntry(
        cls_mod="ragmind_core.processor.simple_txt_processor.SimpleTxtProcessor",
        err="Please install ragmind_core[base] to use TikTokenTxtProcessor ",
    ),
    FileExtension.pdf: ProcEntry(
        cls_mod="ragmind_core.processor.tika_processor.TikaProcessor",
        err=None,
    ),
}
