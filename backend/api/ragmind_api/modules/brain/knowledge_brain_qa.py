...
from ragmind_api.modules.brain.qa_interface import (
    QAInterface,
    model_compatible_with_function_calling,
)
from api.ragmind_api.modules.brain.rags.ragmind_rag import RAGMindRAG
from ragmind_api.modules.brain.rags.rag_interface import RAGInterface
...
from ragmind_api.modules.chat.dto.chats import ChatQuestion, Sources
...
from ragmind_api.modules.prompt.service.get_prompt_to_use import get_prompt_to_use
from ragmind_api.modules.upload.service.generate_file_signed_url import (
...
class KnowledgeBrainQA(BaseModel, QAInterface):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.user_settings = UserSettings(id=user_id, email=user_email)
        self.brain = brain_service.get_brain_by_id(UUID(brain_id))

        # Get Model settings for the user
        self.models_settings = self.user_settings.get_models()
        self.initiate_model()
        self.knowledge_qa = RAGMindRAG(
    ...
    def filter_history(
        ...
        Takes in a chat_history= [HumanMessage(content='Question.'), AIMessage(content="Answer.")]
    ...
    def initiate_model(self):
        model_to_use = find_model_and_generate_metadata(
            self.brain.model, # type: ignore
            self.user_settings.get_user_settings(), # type: ignore
