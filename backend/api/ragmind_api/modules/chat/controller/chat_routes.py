...
from ragmind_api.modules.chat.entity.chat import Chat
...
async def get_answer_generator(
    ...
    brain, metadata_brain = brain_service.find_brain_from_question(
        brain_id, chat_question.question, current_user, chat_id, history, vector_store # type: ignore
    )
...
async def update_chat_message(
    ...
    chat = await chat_service.get_chat_by_id(chat_id) # pyright: ignore reportPrivateUsage=none
    ...
    return chat_service.update_chat_message(
        ...
        chat_message_properties=chat_message_properties.dict(), # type: ignore
    )
...
async def add_question_and_answer_handler(
    ...
    # TODO Do we need to return the chat ??
    return history.chat
