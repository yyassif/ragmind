...
from ragmind_api.modules.brain.entity.brain_entity import Brain, BrainType
from ragmind_api.modules.chat.dto.inputs import QuestionAndAnswer
from ragmind_api.modules.chat.entity.chat import Chat, ChatHistory
from ragmind_api.modules.chat.repository.chats import ChatRepository
from ragmind_api.modules.chat.service.chat_service import ChatService
from ragmind_api.modules.user.entity.user_identity import User
...
async def test_data(
    ...
    # User data
    user_1 = (
        await session.exec(select(User).where(User.email == "admin@yyassif.dev"))
    ).one()
    # Brain data
    brain_1 = Brain(
        name="test_brain",
