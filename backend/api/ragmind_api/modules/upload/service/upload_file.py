...
def check_file_exists(brain_id: str, file_identifier: str) -> bool:
    ...
    try:
        ...
        response = supabase_client.storage.from_("ragmind").list(brain_id)
...
def upload_file_storage(file, file_identifier: str, upsert: str = "false"):
    ...
    try:
        ...
        response = supabase_client.storage.from_("ragmind").upload(
            file_identifier,
            file,
            file_options={
        ...
        if "The resource already exists" in str(e) and upsert == "true":
            response = supabase_client.storage.from_("ragmind").update(
                file_identifier,
                file,
                file_options={
                    "content-type": mime_type,
