...
from ragmind_api.modules.chat.dto.chats import Sources
...
from ragmind_api.packages.ragmind_core.prompts import DEFAULT_DOCUMENT_PROMPT
...
# TODO: CONVOLUTED LOGIC; Redo this
...
def parse_response(raw_response: RawRAGResponse, model_name: str) -> ParsedRAGResponse:
    answer = raw_response["answer"].content # type: ignore
    sources = raw_response["docs"] or []

    metadata = {"sources": sources}

    if model_supports_function_calling(model_name):
        if raw_response["answer"].tool_calls: # type: ignore
            citations = raw_response["answer"].tool_calls[-1]["args"]["citations"] # type: ignore
            metadata["citations"] = citations
            followup_questions = raw_response["answer"].tool_calls[-1]["args"][ # type: ignore
                "followup_questions"
            ]
            thoughts = raw_response["answer"].tool_calls[-1]["args"]["thoughts"] # type: ignore
